...
#if IS_ENABLED(CONFIG_UFS_DATA_LOG_MAGIC_CODE)
 ...
 * 0x05D3D11A26CF3142
 */
#define MAGIC_WORD	0x1F5E3A7069245CBE
#endif
...
static void SEC_ufs_uic_cmd_error_check(struct ufs_hba *hba)
{
	struct SEC_UFS_counting *err_info = &(hba->SEC_err_info);
	struct SEC_UFS_op_count *op_cnt = &(err_info->op_count);
...
static void SEC_ufs_uic_error_check(struct ufs_hba *hba)
{
	struct SEC_UFS_counting *err_info = &(hba->SEC_err_info);
...
static void SEC_ufs_uic_fatal_check(struct ufs_hba *hba)
{
	struct SEC_UFS_counting *err_info = &(hba->SEC_err_info);
...
static void SEC_ufs_utp_error_check(struct ufs_hba *hba, struct scsi_cmnd *cmd)
{
	struct SEC_UFS_counting *err_info = &(hba->SEC_err_info);
...
static void SEC_ufs_tm_error_check(struct ufs_hba *hba, u8 tm_cmd)
{
	struct SEC_UFS_counting *err_info = &(hba->SEC_err_info);
	struct SEC_UFS_UTP_count *utp_err = &(err_info->UTP_count);
...
static void SEC_ufs_query_nop_error_check(struct ufs_hba *hba)
{
	struct SEC_UFS_counting *err_info = &(hba->SEC_err_info);
...
static void SEC_ufs_query_error_check(struct ufs_hba *hba)
{
	struct SEC_UFS_counting *err_info = &(hba->SEC_err_info);
	struct SEC_UFS_QUERY_count *query_cnt = &(err_info->query_count);
 ...
 */
static void SEC_ufs_check_medium_error_region(struct ufs_hba *hba,
...
static void SEC_ufs_sense_err_check(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	struct SEC_UFS_counting *err_info = &(hba->SEC_err_info);
	struct SEC_SCSI_SENSE_count *sense_cnt = &(err_info->sense_count);
	...
	switch (sense_key) {
	case MEDIUM_ERROR:
		SEC_ufs_check_medium_error_region(hba, lrbp);
		SEC_UFS_ERR_COUNT_INC(sense_cnt->scsi_medium_err, UINT_MAX);
#if IS_ENABLED(CONFIG_SEC_ABC)
#if IS_ENABLED(CONFIG_SEC_FACTORY)
		...
		break;
	case HARDWARE_ERROR:
		SEC_UFS_ERR_COUNT_INC(sense_cnt->scsi_hw_err, UINT_MAX);
		/* only work for debug level is mid */
		if (secdbgMode)
...
static void SEC_ufs_print_err_info(struct ufs_hba *hba)
{
	struct SEC_UFS_counting *err_info = &(hba->SEC_err_info);
...
static int ufshcd_device_reset_ctrl(struct ufs_hba *hba, bool ctrl)
{
	...
	if (ret < 0)
		...
		dev_info(hba->dev, "UFS %s is done\n",
...
static void ufshcd_init_hibern8(struct ufs_hba *hba)
{
	...
	if (ufshcd_is_auto_hibern8_supported(hba)) {
		/* Set the default auto-hiberate idle timer value to 2 ms */
...
#if defined(SEC_UFS_ERROR_COUNT)
	if (ret)
		SEC_ufs_uic_cmd_error_check(hba);
#endif
...
#if IS_ENABLED(CONFIG_UFS_DATA_LOG)
#if IS_ENABLED(CONFIG_UFS_DATA_LOG_MAGIC_CODE)
static void ufshcd_store_magic_word(struct scsi_cmnd *cmd,
		unsigned int dump_index)
{
	struct scatterlist *sg;
	int sg_segments;
	int i;
	unsigned long magicword = MAGIC_WORD;
	void *virt_sg_addr;
...
static void ufshcd_check_magic_word(struct scsi_cmnd *cmd, int sg_segments,
...
static inline bool ufshcd_is_data_log_supported(struct ufs_hba *hba)
{
	return hba->host->ufs_sys_log_en;
}

static inline bool ufshcd_is_valid_read_cmd(struct scsi_cmnd *cmd)
{
	return ((cmd->request) &&
...
static inline bool ufshcd_is_ufs_system_part(struct ufs_hba *hba,
		struct scsi_cmnd *cmd)
{
	return (cmd->request->__sector >= hba->host->ufs_system_start &&
			cmd->request->__sector < hba->host->ufs_system_end);
}

static void ufshcd_start_data_log(struct ufs_hba *hba, struct scsi_cmnd *cmd)
{
	unsigned int dump_index;
	int cpu = raw_smp_processor_id();
	int id;

	if (!ufshcd_is_data_log_supported(hba))
		return;

	if (!ufshcd_is_valid_read_cmd(cmd))
		return;

	if (!ufshcd_is_ufs_system_part(hba, cmd))
		return;

	for (id = 0; id < UFS_DATA_LOG_MAX; id++) {
		dump_index = atomic_inc_return(&hba->cur_log_pos) % UFS_DATA_LOG_MAX;
...
#if IS_ENABLED(CONFIG_UFS_DATA_LOG_MAGIC_CODE)
	ufshcd_store_magic_word(cmd, dump_index);
#endif
}

static void ufshcd_end_data_log(struct ufs_hba *hba,
		struct scsi_cmnd *cmd, int tag_index)
{
	int cpu = raw_smp_processor_id();
	unsigned int dump_index;
	int sg_segments;
...
#if IS_ENABLED(CONFIG_UFS_DATA_LOG_MAGIC_CODE)
	ufshcd_check_magic_word(cmd, sg_segments, dump_index, tag_index);
#endif
}
#endif
...
static int ufshcd_link_recovery(struct ufs_hba *hba)
{
	...
	do {
		...
		if (!(work_pending(&hba->eh_work) ||
			...
			break;
		spin_unlock_irqrestore(hba->host->host_lock, flags);
		dev_dbg(hba->dev, "%s %d: reset in progress\n",
				__func__, __LINE__);
...
#if defined(SEC_UFS_ERROR_COUNT)
	if (err)
		SEC_ufs_query_nop_error_check(hba);
#endif
	return err;
}
	...
	case OCS_SUCCESS:
		...
		case UPIU_TRANSACTION_RESPONSE:
			...
			if (scsi_status == SAM_STAT_CHECK_CONDITION)
				SEC_ufs_sense_err_check(hba, lrbp);
#endif
...
#endif
#if IS_ENABLED(CONFIG_UFS_DATA_LOG)
			ufshcd_end_data_log(hba, cmd, index);
#endif
			cmd->result = result;
...
static int ufshcd_abort(struct scsi_cmnd *cmd)
{
	 ...
	 */
#if defined(SEC_UFS_ERROR_COUNT)
	SEC_ufs_utp_error_check(hba, cmd);
#endif
...
#if defined(SEC_UFS_ERROR_COUNT)
			SEC_ufs_tm_error_check(hba, UFS_QUERY_TASK);
#endif
			goto out;
		}
	}
...
#if defined(SEC_UFS_ERROR_COUNT)
		SEC_ufs_tm_error_check(hba, UFS_ABORT_TASK);
#endif
		goto out;
	}
...
static int ufshcd_reset_and_restore(struct ufs_hba *hba)
{
	 ...
	 * for guarantee auto flush before full reset(host and device)
	 */
	ssleep(2);
	ufshcd_enable_irq(hba);
...
#ifdef CONFIG_DEBUG_FS
static void ufshcd_print_hw_reset_ufs_stats(struct ufs_qcom_host *host,
		int *curr_len, char *buf)
{
	int i, len = *curr_len;

	len += snprintf(buf + len, PAGE_SIZE - len, "ufs_stats\n err_stats :");

	for (i = 0; i < UFS_ERR_MAX; i++)
		len += snprintf(buf + len, PAGE_SIZE - len, " %02d:%d", i,
...
static void ufshcd_print_hw_reset_uic_err_hist(struct ufs_uic_err_reg_hist *err_hist,
		int *curr_len, char *buf, char *err_name)
{
	int i, len = *curr_len;
		...
		struct device_attribute *attr, char *buf)
{
	...
	curr_len += snprintf(buf + curr_len, PAGE_SIZE - curr_len,
			"last_time:%lu.\n", host->last_hw_reset);
	curr_len += snprintf(buf + curr_len, PAGE_SIZE - curr_len,
			"saved:0x%x, saved_uic:0x%x.\n",
			host->hw_reset_saved_err, host->hw_reset_saved_uic_err);
	curr_len += snprintf(buf + curr_len, PAGE_SIZE - curr_len,
			"outstanding task:0x%lx, req:0x%lx.\n",
			host->hw_reset_outstanding_tasks,
			host->hw_reset_outstanding_reqs);

	ufshcd_print_hw_reset_ufs_stats(host, &curr_len, buf);

	curr_len += snprintf(buf + curr_len, PAGE_SIZE - curr_len, "ufs_stats\n");

	ufshcd_print_hw_reset_uic_err_hist(&hba->ufs_stats.pa_err,
			&curr_len, buf, "pa_stats");
	ufshcd_print_hw_reset_uic_err_hist(&hba->ufs_stats.dl_err,
			&curr_len, buf, "dl_stats");
	ufshcd_print_hw_reset_uic_err_hist(&hba->ufs_stats.nl_err,
			&curr_len, buf, "nl_stats");
	ufshcd_print_hw_reset_uic_err_hist(&hba->ufs_stats.tl_err,
			&curr_len, buf, "tl_stats");
	ufshcd_print_hw_reset_uic_err_hist(&hba->ufs_stats.dme_err,
			&curr_len, buf, "dme_stats");
...
SEC_UFS_DATA_ATTR_RW(SEC_UFS_op_cnt, "\"HWRESET\":\"%u\",\"LINKFAIL\":\"%u\",\"H8ENTERFAIL\":\"%u\""
		",\"H8EXITFAIL\":\"%u\"\n",
		err_info->op_count.HW_RESET_count,
		err_info->op_count.link_startup_count,
		err_info->op_count.Hibern8_enter_count,
		err_info->op_count.Hibern8_exit_count);
...
SEC_UFS_DATA_ATTR_RW(SEC_UFS_uic_cmd_cnt, "\"TESTMODE\":\"%u\",\"DME_GET\":\"%u\",\"DME_SET\":\"%u\""
		...
		",\"H8ENTER\":\"%u\",\"H8EXIT\":\"%u\"\n",
		err_info->UIC_cmd_count.DME_TEST_MODE_err,
		err_info->UIC_cmd_count.DME_GET_err,
		err_info->UIC_cmd_count.DME_SET_err,
		err_info->UIC_cmd_count.DME_PEER_GET_err,
		err_info->UIC_cmd_count.DME_PEER_SET_err,
		err_info->UIC_cmd_count.DME_POWERON_err,
		err_info->UIC_cmd_count.DME_POWEROFF_err,
		err_info->UIC_cmd_count.DME_ENABLE_err,
		err_info->UIC_cmd_count.DME_RESET_err,
		err_info->UIC_cmd_count.DME_END_PT_RST_err,
		err_info->UIC_cmd_count.DME_LINK_STARTUP_err,
		err_info->UIC_cmd_count.DME_HIBER_ENTER_err,
		err_info->UIC_cmd_count.DME_HIBER_EXIT_err);

SEC_UFS_DATA_ATTR_RW(SEC_UFS_uic_err_cnt, "\"PAERR\":\"%u\",\"DLPAINITERROR\":\"%u\",\"DLNAC\":\"%u\""
		",\"DLTCREPLAY\":\"%u\",\"NLERR\":\"%u\",\"TLERR\":\"%u\",\"DMEERR\":\"%u\"\n",
		err_info->UIC_err_count.PA_ERR_cnt,
		err_info->UIC_err_count.DL_PA_INIT_ERROR_cnt,
		err_info->UIC_err_count.DL_NAC_RECEIVED_ERROR_cnt,
		err_info->UIC_err_count.DL_TC_REPLAY_ERROR_cnt,
		err_info->UIC_err_count.NL_ERROR_cnt,
		err_info->UIC_err_count.TL_ERROR_cnt,
		err_info->UIC_err_count.DME_ERROR_cnt);
...
SEC_UFS_DATA_ATTR_RW(SEC_UFS_utp_cnt, "\"UTMRQTASK\":\"%u\",\"UTMRATASK\":\"%u\",\"UTRR\":\"%u\""
		...
		err_info->UTP_count.UTMR_query_task_count,
		err_info->UTP_count.UTMR_abort_task_count,
		err_info->UTP_count.UTR_read_err,
		err_info->UTP_count.UTR_write_err,
		err_info->UTP_count.UTR_sync_cache_err,
		err_info->UTP_count.UTR_unmap_err,
		err_info->UTP_count.UTR_etc_err);
...
SEC_UFS_DATA_ATTR_RW(SEC_UFS_query_cnt, "\"NOPERR\":\"%u\",\"R_DESC\":\"%u\",\"W_DESC\":\"%u\""
		...
		err_info->query_count.R_Desc_err,
		err_info->query_count.W_Desc_err,
		err_info->query_count.R_Attr_err,
		err_info->query_count.W_Attr_err,
		err_info->query_count.R_Flag_err,
		err_info->query_count.Set_Flag_err,
		err_info->query_count.Clear_Flag_err,
		err_info->query_count.Toggle_Flag_err);

SEC_UFS_DATA_ATTR_RW(sense_err_count, "\"MEDIUM\":\"%u\",\"HWERR\":\"%u\"\n",
		err_info->sense_count.scsi_medium_err,
		err_info->sense_count.scsi_hw_err);
...
SEC_UFS_DATA_ATTR_RO(sense_err_logging, "\"LBA0\":\"%lx\",\"LBA1\":\"%lx\",\"LBA2\":\"%lx\""
		...
		",\"REGIONMAP\":\"%016llx\"\n",
		err_info->sense_err_log.error_LBA_list[0],
		err_info->sense_err_log.error_LBA_list[1],
		err_info->sense_err_log.error_LBA_list[2],
		err_info->sense_err_log.error_LBA_list[3],
		err_info->sense_err_log.error_LBA_list[4],
		err_info->sense_err_log.error_LBA_list[5],
		err_info->sense_err_log.error_LBA_list[6],
		err_info->sense_err_log.error_LBA_list[7],
		err_info->sense_err_log.error_LBA_list[8],
		err_info->sense_err_log.error_LBA_list[9],
		err_info->sense_err_log.error_region_map);
#endif
...
static void ufs_sec_device_create_file(struct device *dev,
...
static void ufs_sec_init_sysfs(struct ufs_hba *hba)
{
	/* sec specific vendor sysfs nodes */
	sec_ufs_cmd_dev = sec_device_create(hba, "ufs");
	if (IS_ERR(sec_ufs_cmd_dev)) {
		pr_err("%s: failed to create sysfs dev\n", __func__);
		return;
	}

	ufs_sec_device_create_file(sec_ufs_cmd_dev, &dev_attr_un);
	ufs_sec_device_create_file(sec_ufs_cmd_dev, &dev_attr_lt);
	ufs_sec_device_create_file(sec_ufs_cmd_dev, &dev_attr_lc);
	ufs_sec_device_create_file(sec_ufs_cmd_dev, &dev_attr_man_id);
	dev_set_drvdata(sec_ufs_cmd_dev, hba);
}
...
extern struct ufs_qcom_host *ufs_qcom_hosts[MAX_UFS_QCOM_HOSTS];
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	memset(&(hba->SEC_err_info_backup), 0, sizeof(struct SEC_UFS_counting));
